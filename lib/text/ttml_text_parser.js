...
shaka.text.TtmlTextParser = class {
  ...
  static parseCue_(
    ...
    if (cueElement.nodeType != Node.ELEMENT_NODE) {
      return null;
    }
    ...
    if (
      /* Disregard empty elements without time attributes nor content
       ...
       */
      (hasNoTimeAttributes && isTextContentEmpty && localWhitespaceTrim) ||
      /*
    ...
    let payload = '';
    const nestedCues = [];
    // If one of the children is a non empty text node,
    ...
    const isNonEmptyText = (node) => {
      ...
      return node.nodeType == Node.TEXT_NODE &&
        ((!localWhitespaceTrim && /\S+|\W+/.test(node.textContent)) ||
          (localWhitespaceTrim && /\S+/.test(node.textContent)));
    };
